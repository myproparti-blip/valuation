...
// CREATE VALUATION (only users can submit forms)
export const createValuation = async (req, res) => {
    try {
        const requestUser = req.user;

        // Authorization: Only users can create submissions
        if (requestUser.role !== "user") {
            return res.status(403).json({ success: false, message: "Forbidden - Only users can submit forms" });
        }
...
export const getAllValuations = async (req, res) => {
    try {
        ...
        // MANAGER & ADMIN: Can see user valuations with role-based filtering
            ...
            if (requestUser.role === "manager1") {
                ...
                // admin: Can see all user valuations
