...
// Helper to add simple text
function addText(text, size = 9, bold = false, color = '#000000', align = 'left') {
  doc.font(bold ? 'Helvetica-Bold' : 'Helvetica').fontSize(size).fillColor(color);
  const options = { width: contentWidth, align };
  const height = doc.heightOfString(text, options);
  doc.text(text, margin, yPos, options);
  yPos += height + 3;
  return height;
}

function addSpacing(height = 10) {
  yPos += height;
}
...
yPos += 10;
doc.text('Gujarat Gramin Bank , Vadodara', margin, yPos);
yPos += 10;
doc.text('Manjalpur Branch', margin, yPos);

// Right align header
doc.fontSize(9).text('File No: 06GGB1025 10', margin, yPos - 20, { width: contentWidth, align: 'right' });
doc.text('Date: 31-Oct-2025', margin, yPos - 10, { width: contentWidth, align: 'right' });

yPos += 20;

// Title
addText('VALUATION REPORT(IN RESPECT OF FLAT/HOUSE/INDUSTRIAL/SHOP)', 11, true, '#000000', 'center');

addSpacing(15);

// Table function
function drawTable(data, colWidths, options = {}) {
  const { headerBgColor = '#003366', headerTextColor = '#FFFFFF', fontSize = 8, headerFontSize = 9 } = options;
  ...
  data.forEach((row, rowIdx) => {
    ...
    row.forEach((cell, colIdx) => {
      ...
      const isHeader = rowIdx === 0 && options.isHeader;
      
      // Border
      doc.strokeColor('#000000').lineWidth(0.5);
      doc.rect(x, tableY, width, cellHeight).stroke();
      
      // Background
      if (isHeader) {
        doc.fillColor(headerBgColor).rect(x, tableY, width, cellHeight).fill();
      }
      
      // Text
      doc.fillColor(isHeader ? headerTextColor : '#000000');
      ...
      const cellTextHeight = doc.heightOfString(cell, textOptions);
      const textY = tableY + (cellHeight - cellTextHeight) / 2;
      
      doc.text(cell, x + 4, textY, textOptions);
...
drawTable(generalHeader, colWidths, { isHeader: true });
generalData.forEach(row => {
  drawTable([row], colWidths, { isHeader: false });
});
...
addText('II. APARTMENT BUILDING', 10, true);
addSpacing(8);
...
// IV MARKETIBILITY
addText('IV MARKETIBILITY', 10, true);
addSpacing(8);
...
addText('VI COMPOSITE RATE ADOPTED AFTER DEPRECIATION', 10, true);
addSpacing(8);
...
addText('DETAILS OF VALUATION', 10, true);
addSpacing(8);
...
// As a result
addText('As a result of my appraisal and analysis,', 9, true);
addSpacing(10);

const finalColWidths = [320, 225];
const finalData = [
...
finalData.forEach(row => {
  ...
  drawTable([row], finalColWidths, { isHeader: false });
});
...
doc.text('STATEMENT OF LIMITING CONDITIONS', margin, yPos, { width: contentWidth });
yPos += 20;
...
conditions.forEach(cond => {
  const h = doc.heightOfString(cond, { width: contentWidth });
  doc.text(cond, margin, yPos, { width: contentWidth });
  yPos += h + 5;
});
...
doc.text('VIII DECLARATION', margin, yPos, { width: contentWidth });
yPos += 20;
...
declData.forEach(row => {
  if (yPos > 700) {
    doc.addPage();
    yPos = margin;
  }
  drawTable([row], declColWidths, { isHeader: false });
});
...
doc.text('Place: Vadodara', margin, yPos);
doc.text('Date: 31/10/2025', margin, yPos + 20);

doc.font('Helvetica-Bold').fontSize(9);
doc.text('SIGNATURE OF THE VALUER', margin + 320, yPos, { width: 200, align: 'center' });
doc.text('MAHIM ARCHITECTS', margin + 320, yPos + 20, { width: 200, align: 'center' });

yPos += 60;

// Enclosures
doc.font('Helvetica-Bold').fontSize(9).fillColor('#000000');
doc.text('Enclsd: 1. Declaration from the valuer', margin, yPos);
...
const enclHeight = doc.heightOfString(enclText, { width: contentWidth });
doc.text(enclText, margin, yPos, { width: contentWidth });

yPos += enclHeight + 30;

doc.font('Helvetica-Bold').fontSize(9);
doc.text('SIGNATURE', margin + 320, yPos, { width: 200, align: 'center' });
doc.text('NAME OF BRANCH OFFICIAL WITH SEAL', margin + 320, yPos + 20, { width: 200, align: 'center' });
...
stream.on('finish', () => {
  ...
  console.log('✓ This is an exact replica of the valuation report');
});
