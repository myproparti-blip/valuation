...
const LoginPage = ({ onLogin }) => {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#F36E21] via-[#EC5E25] to-[#7A1F14] p-4 relative overflow-hidden">
            {/* Decorative elements */}
            <div className="absolute top-0 right-0 w-96 h-96 bg-white/5 rounded-full blur-3xl"></div>
            <div className="absolute bottom-0 left-0 w-96 h-96 bg-white/5 rounded-full blur-3xl"></div>

            <Card className="w-full max-w-md shadow-2xl overflow-hidden border-0 relative z-10">
                <CardHeader className="text-center space-y-3 bg-gradient-to-br from-slate-50 to-orange-50 py-6 sm:py-8 border-b-4 border-[#F36E21]">
                    <div className="flex justify-center mb-2">
                        <div className="text-5xl sm:text-6xl filter drop-shadow-lg">📊</div>
                    </div>
                    <div>
                        <CardTitle className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-[#F36E21] to-[#7A1F14] bg-clip-text text-transparent">VALUE SOFT</CardTitle>
                    ...
                    <form onSubmit={onFinish} className="space-y-5">
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="username" className="font-semibold text-slate-700 flex items-center gap-2">
                                <FaEnvelope className="h-4 w-4 text-[#F36E21]" />
                                Username
                            </Label>
                            <Input
                                id="username"
                                ...
                                disabled={loading}
                                className="pl-4 h-11 font-medium border-2 focus:border-[#F36E21] focus:shadow-lg"
                                required
                            />
                        </div>
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="password" className="font-semibold text-slate-700 flex items-center gap-2">
                                <FaLock className="h-4 w-4 text-[#F36E21]" />
                                Password
                            </Label>
                            <div className="relative">
                                <Input
                                    ...
                                    onChange={handleInputChange}
                                    disabled={loading}
                                    className="pr-10 h-11 font-medium border-2 focus:border-[#F36E21] focus:shadow-lg"
                                    required
                                />
                                <button
                                    type="button"
                                    onClick={() => setShowPassword(!showPassword)}
                                    className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-[#F36E21] transition-colors"
                                    disabled={loading}
                                >
                                    {showPassword ? (
