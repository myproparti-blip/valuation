...
const stream = fs.createWriteStream('Valuation_Report_Exact.pdf');
doc.pipe(stream);
...
function drawLine(x, y, width, color = '#000000', lineWidth = 0.5) {
  doc.strokeColor(color).lineWidth(lineWidth);
  doc.moveTo(x, y).lineTo(x + width, y).stroke();
}

function drawRect(x, y, width, height, color = '#000000', lineWidth = 0.5, fill = null) {
  doc.strokeColor(color).lineWidth(lineWidth);
  if (fill) {
    doc.fillColor(fill).rect(x, y, width, height).fillAndStroke();
  } else {
...
function drawTable(rows, colWidths, options = {}) {
  ...
  currentY = tableY + (rows.length * rowHeight);
}

function addSectionTitle(title, fontSize = 10, bold = true, color = '#000000') {
  doc.font(bold ? 'Helvetica-Bold' : 'Helvetica').fontSize(fontSize).fillColor(color);
  doc.text(title, MARGIN, currentY);
  currentY += fontSize + 8;
}

function addBulletPoint(text, fontSize = 8, indent = 10) {
  doc.font('Helvetica').fontSize(fontSize).fillColor('#000000');
  const bulletWidth = CONTENT_WIDTH - indent;
  const height = doc.heightOfString(text, { width: bulletWidth });
  doc.text('•', MARGIN + 5, currentY);
  doc.text(text, MARGIN + indent, currentY, { width: bulletWidth });
  currentY += height + 4;
}
...
function checkPageBreak(neededHeight = 100) {
...
currentY += 12;
doc.text('Gujarat Gramin Bank , Vadodara', MARGIN, currentY);
currentY += 12;
doc.text('Manjalpur Branch', MARGIN, currentY);

// Right-aligned header
const headerStartY = currentY - 24;
doc.text('File No: 06GGB1025 10', MARGIN, headerStartY, { width: CONTENT_WIDTH - 10, align: 'right' });
doc.text('Date: 31-Oct-2025', MARGIN, headerStartY + 12, { width: CONTENT_WIDTH - 10, align: 'right' });
...
doc.text('VALUATION REPORT(IN RESPECT OF FLAT/HOUSE/INDUSTRIAL/SHOP)', MARGIN, currentY, {
  width: CONTENT_WIDTH,
  align: 'center'
});
currentY += 18;
...
doc.text('As a result of my appraisal and analysis,', MARGIN, currentY);
currentY += 16;
...
doc.text('STATEMENT OF LIMITING CONDITIONS', MARGIN, currentY);
currentY += 14;
...
// VIII DECLARATION
doc.font('Helvetica-Bold').fontSize(10).fillColor('#000000');
doc.text('VIII DECLARATION', MARGIN, currentY);
currentY += 14;
...
// Signature section
doc.font('Helvetica').fontSize(9).fillColor('#000000');
doc.text('Place: Vadodara', MARGIN, currentY);
currentY += 12;
doc.text('Date: 31/10/2025', MARGIN, currentY);

const sigY = currentY;
doc.font('Helvetica-Bold').fontSize(9);
doc.text('SIGNATURE OF THE VALUER', MARGIN + 320, sigY, { width: 200, align: 'center' });
doc.text('MAHIM ARCHITECTS', MARGIN + 320, sigY + 14, { width: 200, align: 'center' });

currentY += 35;

// Enclosure
doc.font('Helvetica-Bold').fontSize(9).fillColor('#000000');
doc.text('Enclsd: 1. Declaration from the valuer', MARGIN, currentY);
currentY += 14;
...
doc.text(enclText, MARGIN, currentY, { width: CONTENT_WIDTH });
currentY += enclH + 18;

doc.font('Helvetica-Bold').fontSize(9);
doc.text('SIGNATURE', MARGIN + 320, currentY, { width: 200, align: 'center' });
currentY += 12;
doc.text('NAME OF BRANCH OFFICIAL WITH SEAL', MARGIN + 320, currentY, { width: 200, align: 'center' });
...
stream.on('finish', () => {
  ...
  console.log('✓ File: Valuation_Report_Exact.pdf');
});
